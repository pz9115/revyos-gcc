...
static const struct riscv_builtin_description riscv_builtins[] = {
  ...
  _RVV_MASK_ITERATOR_ARG (MASK_LOGICAL_BUILTINS, andn)
  _RVV_MASK_ITERATOR_ARG (MASK_LOGICAL_BUILTINS, orn)
...
/* Return true if TYPE is a built-in RVV type.  */
bool
builtin_vector_type_p (const_tree type)
{
		      ...
		      int ignore ATTRIBUTE_UNUSED)
{
  ...
  if (d->avail == riscv_builtin_avail_vector
      && target_subset_version_p ("v", 0, 7)
      && !check_vector_v0p7 (exp, target, subtarget, mode))
    return const0_rtx;

  /* If there are some vector builtin calls, then we set has_vector_ops_p
     to true and reinit regs info.  */
  if (d->avail == riscv_builtin_avail_vector
      && (riscv_insn_dread_vlenb || target_subset_version_p ("v", 0, 7))
...
DEF_CHECKER (vector_v0p7)
{
  bool ret = true;
  tree fndecl = get_callee_fndecl (exp);
  const char *fname = (const char *)DECL_NAME (fndecl)->identifier.id.str;

  regex_t pat;
  regmatch_t match;
  if (!regcomp (&pat, "mf[1-9]", REG_EXTENDED)
      && !regexec (&pat, fname, 1, &match, 0))
    ret = false;

  for (int argno = 0; ret && argno < call_expr_nargs (exp); argno++)
    {
      tree arg = CALL_EXPR_ARG (exp, argno);
      machine_mode mode = TYPE_MODE (TREE_TYPE (arg));
      if (VECTOR_MODE_P (mode)
	  && known_lt (GET_MODE_SIZE (mode), BYTES_PER_RVV_VECTOR))
	ret = false;
    }
