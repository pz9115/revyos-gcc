...
static const struct attribute_spec riscv_attribute_table[] =
{
  ...
  /* This attribute defines RVV type information.  */
  { "RVV type",	1, 1, false, true,  false, true,  NULL, NULL },
...
riscv_float_const_rtx_index_for_fli (rtx x)
{
  ...
  if (!TARGET_RVZFA || mode == VOIDmode
      || !CONST_DOUBLE_P(x)
      || (mode == HFmode && !TARGET_FP16)
...
riscv_compressed_reg_p (int regno)
{
  ...
  return ((TARGET_RVC || TARGET_ZCA) && (IN_RANGE (regno, GP_REG_FIRST + 8, GP_REG_FIRST + 15)
	  ...
	  if (SMALL_OPERAND (((offset_value < 0) ? offset_value - 1 : offset_value + 1) / 2))
	    {
		 ...
		 use addi + ld/st instead of li + add + ld/st.  */
	      const int base = (offset_value < 0) ? -2048 : 2047;
	      high = gen_int_mode (base, Pmode);
	      offset_value = offset_value - base;
	      reg = riscv_force_temporary (temp, gen_rtx_PLUS (Pmode, reg, high),
					   FALSE);
	    }
	  else
	    {
		 ...
		 int *total, bool speed)
{
  ...
  if (riscv_xthead_rtx_costs (x, mode, outer_code, opno, total, speed))
    return true;
		    ...
		    bool speed ATTRIBUTE_UNUSED)
{
  ...
  if ((TARGET_RVC || TARGET_ZCA) && !speed && riscv_mshorten_memrefs && mode == SImode
...
riscv_split_64bit_move_p (rtx dest, rtx src)
{
  ...
  if (TARGET_RVZFA && (riscv_float_const_rtx_index_for_fli (src) != -1))
    return false;

  if (TARGET_XTHEAD_FLDR)
    {
      if (MEM_P (src) && SCALAR_FLOAT_MODE_P (GET_MODE (src))
	  && riscv_legitimize_address_index_p (XEXP (src, 0), GET_MODE (src), false)
	  && FP_REG_RTX_P (dest))
	return false;
      if (MEM_P (dest) && SCALAR_FLOAT_MODE_P (GET_MODE (dest))
	  && riscv_legitimize_address_index_p (XEXP (dest, 0), GET_MODE (dest), false)
	  && FP_REG_RTX_P (src))
	return false;
    }
  ...
  if (TARGET_DOUBLE_FLOAT
	  ...
	  || ((TARGET_XTHEAD_FMVHW32 || TARGET_RVZFA)
	      && ((FP_REG_RTX_P (dest) && GP_REG_RTX_P (src)) || (FP_REG_RTX_P (src) && GP_REG_RTX_P (dest))))
...
riscv_output_move (rtx dest, rtx src)
{
  ...
  if (dest_code == REG && GP_REG_P (REGNO (dest)))
    {
      if ((TARGET_XTHEAD_FMVHW32 || TARGET_RVZFA)
      ...
      if (src_code == MEM)
	{
	  ...
	  if (!insn)
	    insn = riscv_output_move_modify (XEXP (src, 0), GET_MODE (src), true);
	  if (insn)
	    return insn;
  ...
  if ((src_code == REG && GP_REG_P (REGNO (src)))
	...
	{
	  const char *insn = NULL;
	  insn = riscv_output_move_index (XEXP (dest, 0), GET_MODE (dest), false);
	  if (!insn)
	    insn = riscv_output_move_modify (XEXP (dest, 0), GET_MODE (dest), false);
	  if (insn)
	    return insn;
  ...
  if (src_code == REG && FP_REG_P (REGNO (src)))
	...
	{
	  const char *insn = NULL;
	  insn = riscv_output_move_index_float (XEXP (dest, 0), GET_MODE (dest), false);
	  if (insn)
	    return insn;
	switch (width)
	  {
  ...
  if (dest_code == REG && FP_REG_P (REGNO (dest)))
	...
	{
	  const char *insn = NULL;
	  insn = riscv_output_move_index_float (XEXP (src, 0), GET_MODE (src), true);
	  if (insn)
	    return insn;
	switch (width)
	  {
  ...
  if (dest_code == REG && GP_REG_P (REGNO (dest)) && src_code == CONST_POLY_INT)
      ...
      if (target_subset_version_p ("v", 0, 7))
	{
	  ...
	  const char *format = "csrr\tx%d,vl\n\tcsrr\tx%d,vtype\n\tvsetvli\t%%0,x0,e8,m1\n\tvsetvl\tx0,x%d,x%d\t#csrr\t%%0,vlenb";
	  if (riscv_rvv_vector_bits != RVV_SCALABLE)
	    snprintf (buf, sizeof (buf), "li\t%%0,%d\t#csrr\t%%0,vlenb", riscv_rvv_vector_bits / 8);
	  else
	    snprintf (buf, sizeof (buf), format,
		      TARGET_VECTOR_TEMP2_REGNUM, TARGET_VECTOR_TEMP_REGNUM, TARGET_VECTOR_TEMP2_REGNUM, TARGET_VECTOR_TEMP_REGNUM);
	  return buf;
	}
      return "csrr\t%0,vlenb";
    }
  ...
  switch (TREE_CODE (type))
    ...
    default:
	  ...
	  && ((SCALAR_FLOAT_TYPE_P (type)
		   ...
		   <= UNITS_PER_FP_ARG))
	      || ((INTEGRAL_TYPE_P (type) || riscv_dsp_mode (TYPE_MODE (type)))
		  && (GET_MODE_SIZE (TYPE_MODE (type)).to_constant ()
		      <= UNITS_PER_WORD))))
	{
	  fields[n].type = type;
  ...
  for (int i = 0; i < n_old; i++)
    {
      ...
      num_int_old += (INTEGRAL_TYPE_P (fields[i].type) || riscv_dsp_mode (TYPE_MODE (fields[i].type)));
    }
  ...
  for (int i = 0; i < n_new; i++)
    {
      ...
      num_int_new += (INTEGRAL_TYPE_P (fields[i].type) || riscv_dsp_mode (TYPE_MODE (fields[i].type)));
    }
...
riscv_pass_by_reference (cumulative_args_t cum_v, const function_arg_info &arg)
{
  ...
  if (TARGET_VECTOR_FIXED && riscv_vector_mode (arg.mode))
    return true;
  ...
  if (is_attribute_p ("interrupt", name))
      ...
      while (args != NULL)
	{
	  ...
	  if (strcmp (string, "user") && strcmp (string, "supervisor")
	      ...
	      && strcmp (string, "THead-interrupt-nesting"))
	    {
	      warning (OPT_Wattributes,
...
riscv_save_reg_p (unsigned int regno)
{
  ...
  if (regno == 27 && TARGET_XTHEAD_DREAD_VLENB)
    return true;
...
riscv_compute_frame_info (void)
{
  ...
  if (!cfun->machine->naked_p)
	...
	if (riscv_save_reg_p (regno)
	    ...
	    if (TARGET_XTHEAD_INTERRUPT_NESTING_IPUSH && riscv_save_reg_ipush_p (regno))
	      frame->imask |= 1 << (regno - GP_REG_FIRST), num_x_isaved++;
          }
  ...
  if (cfun->machine->interrupt_handler_p)
    {
      ...
      /* TODO: When V extensions support interrupts, some of their CSRs
	 may also need to be saved and restored.  */
      if (TARGET_HARD_FLOAT && frame->fmask)
	/* Save and restore FCSR for float.  */
	/* TODO: If the TARGET_ZFINX is supported, frame->mask needs to be considered here.*/
	csr_offset += 1 * UNITS_PER_WORD;

      if (TARGET_XTHEAD_DSP)
	/* Save and restore VXSAT for P-EXT.  */
	csr_offset += 1 * UNITS_PER_WORD;

      if (TARGET_XTHEAD_INTERRUPT_NESTING_IPUSH && riscv_insert_ipush_p ())
	{
	  if (csr_offset > 0)
	    /* Avoid ipush causing failure to generate using RISCV_PROLOGUE_TEMP_REGNUM.  */
	    frame->imask &= ~(1 << (RISCV_PROLOGUE_TEMP_REGNUM));
	  frame->save_ipush_adjustment = riscv_stack_align (num_x_isaved * UNITS_PER_WORD);
	  offset -= frame->save_ipush_adjustment;
	}
      else if (TARGET_XTHEAD_INTERRUPT_NESTING_CSR)
	  /* Save and restore MCAUSE, MEPC, MSTATUS.  */
	  csr_offset += 3 * UNITS_PER_WORD;

      if (csr_offset >0)
	{
	  csr_offset += 1 * UNITS_PER_WORD;
	  cfun->machine->interrupt_flags.csr_offset = csr_offset;
	  offset = riscv_stack_align (offset + csr_offset);
	}
    }
...
static bool
riscv_handle_csr_for_interrupt (HOST_WIDE_INT offset, riscv_save_restore_fn fn,
				bool epilogue, int regno);
			  ...
			  bool epilogue, bool maybe_eh_return)
{
  ...
  offset = (cfun->machine->frame.gp_sp_offset - sp_offset).to_constant () - cfun->machine->interrupt_flags.csr_offset;
  ...
  if ((frame->mask | frame->fmask) != 0)
	...
	step1 = MIN (size.to_constant(), step1);
     ...
     size -= step1;
      ...
      if (cfun->machine->interrupt_handler_p
	  && riscv_handle_csr_for_interrupt ((cfun->machine->frame.gp_sp_offset - size).to_constant (),
					     riscv_save_reg, false, RISCV_PROLOGUE_TEMP_REGNUM))
	cfun->machine->frame.mask &= ~ (1 << (RISCV_PROLOGUE_TEMP_REGNUM - GP_REG_FIRST));

      riscv_for_each_saved_reg (size, riscv_save_reg,
				false, false);

      if (TARGET_XTHEAD_INTERRUPT_NESTING_CSR)
	/* Re-enable interrupts.  */
	emit_insn (gen_riscv_csr_set_bits (GEN_INT (MSTATUS_REGNUM), GEN_INT (MSTATUS_MIE_BIT)));
    }
...
riscv_expand_epilogue (int style)
{
  ...
  if (cfun->machine->interrupt_handler_p
      && riscv_handle_csr_for_interrupt ((cfun->machine->frame.gp_sp_offset - frame->total_size).to_constant () + step2,
					 riscv_restore_reg, true, RISCV_PROLOGUE_TEMP_REGNUM))
    cfun->machine->frame.mask &= ~ (1 << (RISCV_PROLOGUE_TEMP_REGNUM - GP_REG_FIRST));

  /* Restore the registers.  */
  riscv_for_each_saved_reg (frame->total_size - step2,
			  ...
			  reg_class_t from, reg_class_t to)
{
  ...
  if (((!TARGET_FP16 && mode == HFmode) || (!TARGET_BF16 && mode == BFmode))
...
/* Return true if MODE is vector mode.  */
...
riscv_vector_mode (machine_mode mode)
{
  ...
  if (VECTOR_MODE_P (mode)
      && ((TARGET_VECTOR_FIXED && GET_MODE_SIZE (mode).is_constant ())
	  || (!TARGET_VECTOR_FIXED && !GET_MODE_SIZE (mode).is_constant ()))
      && (inner == BImode
	  || inner == QImode
	  || inner == HImode
...
riscv_hard_regno_nregs (unsigned int regno, machine_mode mode)
{
  ...
  if (GP_REG_P (regno))
    ...
    return CEIL (lowest_size, UNITS_PER_FP_REG);
...
riscv_hard_regno_mode_ok (unsigned int regno, machine_mode mode)
{
  ...
  if (GP_REG_P (regno))
	...
	return false;
	  ...
	  && !TARGET_64BIT && nregs == 2 && regno % 2 !=0)
	return false;

      if (VECTOR_MODE_P (mode) && !riscv_dsp_mode (mode))
	return false;
    }
  else if (FP_REG_P (regno))
    {
...
{
  if (TARGET_VECTOR_FIXED && (riscv_vector_mode (mode1) != riscv_vector_mode (mode2)))
    return false;
...
riscv_option_override (void)
{
  ...
  if (riscv_abi == ABI_ILP32E && TARGET_DOUBLE_FLOAT)
    error ("RV32E must not be used with the D ISA extension");
...
riscv_conditional_register_usage (void)
{
  ...
  if (target_subset_version_p ("v", 0, 7)
    ...
    {
      fixed_regs[TARGET_VECTOR_TEMP_REGNUM] = call_used_regs[TARGET_VECTOR_TEMP_REGNUM] = 1;
      fixed_regs[TARGET_VECTOR_TEMP2_REGNUM] = call_used_regs[TARGET_VECTOR_TEMP2_REGNUM] = 1;
    }

  /* When deleting redundant reads of vlenb, we need a register to store
     the value of vlenb, here we use register s11(x27).  */

  /* For first init, cfun is null.  */
  if (cfun && TARGET_XTHEAD_DREAD_VLENB)
...
riscv_get_interrupt_type (tree decl)
{
  ...
  interrupt_flags.thead_clic_preemptible_p = FALSE;
  ...
  while (attr_args)
    {
      ...
      if (!strcmp (string, "THead-interrupt-nesting"))
	interrupt_flags.thead_clic_preemptible_p = TRUE;
      else
	{
...
riscv_set_current_function (tree decl)
{
  ...
  if (cfun->machine->interrupt_handler_p)
	...
	{
	  if (cfun->machine->interrupt_flags.thead_clic_preemptible_p)
	    error ("THEAD CLIC preemptible %qs function must be machine mode",
		   "interrupt");
	}
...
{
  if ((TARGET_XTHEAD_ZPSFOPERAND || TARGET_XTHEAD_ZPN)
      && reg_class_subset_p (rclass, GR_REGS))
    return true;
...
riscv_reorg (void)
{
  ...
  if (TARGET_XTHEAD_FCLASS && !TARGET_RVZFA)
...
{
  if (lookup_attribute ("RVV type", TYPE_ATTRIBUTES (type1))
      != lookup_attribute ("RVV type", TYPE_ATTRIBUTES (type2)))
    return 0;
  return 1;
}
...
riscv_preferred_simd_mode (scalar_mode mode)
{
  ...
  if (riscv_dsp_preferred_mode (mode) != VOIDmode)
    return riscv_dsp_preferred_mode (mode);
...
riscv_vector_alignment (const_tree type)
{
  ...
  if (TARGET_VECTOR_FIXED && VECTOR_MODE_P (TYPE_MODE (type)))
    return align;
...
{
  return builtin_vector_type_p (type1) == builtin_vector_type_p (type2);
}
...
static const char *
riscv_invalid_binary_op (int op ATTRIBUTE_UNUSED, const_tree type1,
			   const_tree type2)
{
  if (VECTOR_TYPE_P (type1)
      ...
      && (builtin_vector_type_p (type1)
	  != builtin_vector_type_p (type2)))
    return "cannot combine GNU and RVV vectors in a binary operation";
...
riscv_asan_shadow_offset (void)
{
  /* We only have libsanitizer support for RV64 at present.
     ...
     libsanitizer/asan/asan_mapping.h which is currently 0xd55550000 
     for rv64, even though 1<<36 makes more sense.  */
...
#define TARGET_INVALID_BINARY_OP riscv_invalid_binary_op
