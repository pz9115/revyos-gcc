...
#define RVV_FCVT_TEST(STYPE, VCLASST, VCLASS, EM, MLEN, ISTYPE, IVCLASST, IVCLASS, SEW)	\
  ...
  {									\
    ...
    vy = vfcvt_x_f_v_i##EM##_m (mask, vy, vx, vl);	\
  ...
  {									\
    ...
    vx = VLOAD(VCLASS, SEW, EM, x);					\
    vy = VULOAD(SEW, EM, y);					\
    vy = vfcvt_xu_f_v_u##EM##_m (mask, vy, vx, vl);	\
    VUSTORE(SEW, EM, y, vy);					\
  }									\
  ...
  {									\
    ...
    vx = VLOAD(VCLASS, SEW, EM, x);					\
    vy = VLOAD(IVCLASS, SEW, EM, y);					\
    vy = vfcvt_rtz_x_f_v_i##EM##_m (mask, vy, vx, vl);	\
    VSTORE(IVCLASS, SEW, EM, y, vy);					\
  }									\
  ...
  {									\
    ...
    vx = VLOAD(VCLASS, SEW, EM, x);					\
    vy = VULOAD(SEW, EM, y);					\
    vy = vfcvt_rtz_xu_f_v_u##EM##_m (mask, vy, vx, vl);	\
    VUSTORE(SEW, EM, y, vy);					\
  }									\
  ...
  {									\
    ...
    vx = VLOAD(VCLASS, SEW, EM, x);					\
    vy = VLOAD(IVCLASS, SEW, EM, y);					\
    vx = vfcvt_f_x_v_f##EM##_m (mask, vx, vy, vl);	\
  ...
  {									\
    ...
    vx = VLOAD(VCLASS, SEW, EM, x);					\
    vy = VULOAD(SEW, EM, y);					\
    vx = vfcvt_f_xu_v_f##EM##_m (mask, vx, vy, vl);	\
    VSTORE(VCLASS, SEW, EM, x, vx);						\
  }									\
