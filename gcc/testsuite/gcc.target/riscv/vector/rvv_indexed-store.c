...
#define VSLOADSTORE(STYPE, VCLASST, VCLASS, EM, MLEN, SEW, IEM, ISEW, OP)             \
  void vsloadstore##ISEW##VCLASS##EM(size_t n, long stride, STYPE *x,             \
    ...
    vsoxe##i##ISEW##_v_##VCLASS##EM(x, vindex, vz, vl);                                   \
  }									          \
  void vsuloadstore##ISEW##VCLASS##EM(size_t n, long stride, STYPE *x,            \
    ...
    vy = VLOAD(VCLASS, SEW, EM, y);                                                   \
    vz = v##OP##_vv_##VCLASS##EM (vx, vy, vl);                                      \
    vsuxe##i##ISEW##_v_##VCLASS##EM(x, vindex, vz, vl);                                  \
  }

RVV_INT_INDEX_TEST_ARG(VSLOADSTORE, add)
RVV_UINT_INDEX_TEST_ARG(VSLOADSTORE, add)
RVV_FLOAT_INDEX_TEST_ARG(VSLOADSTORE, fadd)
