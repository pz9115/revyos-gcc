...
#define RVV_NFCVT_TEST(EM, WEM, MLEN, SEW, WSEW)	\
  ...
  {									\
    ...
    vint##EM##_t vy;							\
    vx = VFLOAD(WSEW, WEM, x);						\
    vy = vfncvt_rtz_x_f_w_i##EM (vx, vl);			\
    VISTORE(SEW, EM, y, vy);						\
  }									\
  ...
  {									\
    ...
    vuint##EM##_t vy;						\
    vx = VFLOAD(WSEW, WEM, x);					\
    vy = vfncvt_rtz_xu_f_w_u##EM (vx, vl);			\
    VUSTORE(SEW, EM, y, vy);					\
  }									\
  ...
  {									\
    ...
    vfloat##WEM##_t vy;						\
    vy = VFLOAD(WSEW, WEM, y);						\
    vx = vfncvt_rod_f_f_w_f##EM (vy, vl);			\
    VFSTORE(SEW, EM, x, vx);						\
  }
